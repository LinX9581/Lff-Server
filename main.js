...
app.post('/api_achievement_list_undertake', (req, res) => {
    ...
    try {
        ...
        var query_string = "SELECT achievement.*, IFNULL(achievement_task.status,'undertake') as status FROM achievement LEFT JOIN achievement_task on achievement.activity_id = achievement_task.activity_id AND achievement_task.user_id= ? WHERE achievement_task.activity_id IS NULL  AND achievement.category = ? " //找出未進行的成就(可承接)
        var obj = []
        sql = mysql.format(query_string, inserts);
        conn.query(sql, function (error, row, field) {
            if (error) {
...
app.post('/api_create_post', (req, res) => {
    ...
    try {
        ...
        var query_string = "INSERT INTO `post`(`activity_id`,`user_id`, `title`, `content`, `image_uri`) VALUES (? ,?, ?, ?, ? )" 
        var uri = user_id + '/' + activity_id + '_' + user_id + '.jpg'
        var inserts = [activity_id, user_id, title, content, uri];
        sql = mysql.format(query_string, inserts);
...
app.post('/api_check_task_status', (req, res) => {// 找出使用者進行中並已經完成條件的任務

    try {
        var user = req.body.user;
        var query_string = "SELECT post.post_id, post.activity_id, COUNT(statistic_praise.praise) as praise FROM post LEFT JOIN statistic_praise ON post.post_id = statistic_praise.post_id WHERE post.user_id= ? GROUP BY(post.post_id) HAVING praise>=2"
        var inserts = [user];
        var eligible = []; //儲存這個使用者已符合完成資格的"所有任務"
        sql = mysql.format(query_string, inserts);
        conn.query(sql, function (error, row, field) {
            if (error) {
                console.log(error);
            ...
            query_string = "SELECT activity_id FROM `achievement_task` WHERE user_id= ?  AND status = 'processing' " 
            sql = mysql.format(query_string, inserts);
            conn.query(sql, function (error, row, field) {
                if (error) {
                    console.log(error);
...
app.post('/api_set_achievement_compelted', (req, res) => {
    ...
    try {
        ...
        var query_string = "INSERT INTO `achievement_task`(`activity_id`, `user_id`, `status`) VALUES ( ?, ?, 'complete')" //新增貼文    
...
app.post('/api_get_personal_post', (req, res) => {
    ...
    try {
        var user_id = req.body.user;      
        var pressed_which = "SELECT post.post_id FROM statistic_praise INNER JOIN post ON post.post_id=statistic_praise.post_id WHERE post.user_id = ? and statistic_praise.giver = ? "
        ...
        conn.query(sql, function (error, row, field) {
            ...
            conn.query(sql, function (error, row, field) {
                ...
                var obj = []
                for (var i in row) {            
                    obj.push({ 'post_id': row[i].post_id, 'user_id': row[i].user_id, 'title': row[i].title, 'content': row[i].content, 'image_uri': row[i].image_uri, 'praise': row[i].praise, 'date': row[i].date, 'flag': false })
                }
                res.send({ 'message': obj, 'pressed_which': result });
            })
        })
...
app.post('/api_get_random_post', (req, res) => {
    ...
    try {        
        var user_id = req.body.user;
        var query_string = "SELECT post_id FROM statistic_praise WHERE giver = ? "  
        var inserts = user_id;
        var clicked = [];
        var obj = [];
        sql = mysql.format(query_string, inserts);
            ...
            if (row.length > 0) {
                ...
                for (let i in row) {  
                    ...
                    conn.query(sql, function (error, banana, field) {                     
                        obj.push({ 'activity_id': banana[0].activity_id, 'describe': banana[0].describe_medal }) //勳章資訊
...
app.post('/api_confirm_this_location', (req, res) => { //確認這個地點完成了沒
